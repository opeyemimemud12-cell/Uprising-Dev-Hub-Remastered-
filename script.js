...
async function saveGame(game) {
  ...
  // ── Try Firebase Storage upload (with hard timeout so it never hangs) ──
  if (fbReady && db && storage && !isLocalFile && game._blob) {
    try {
      showProgress('Uploading image…', 15);

      const imageUrl = await new Promise((resolve, reject) => {
        // 30 second hard timeout — if Storage hangs, we fall through to dataURL
        ...
        uploadTask.on('state_changed',
          snap => {
            if (snap.totalBytes > 0) {
              const pct = Math.round((snap.bytesTransferred / snap.totalBytes) * 65) + 15;
              showProgress('Uploading image…', pct);
            }
          },
      ...
      // Don't alert on timeout — just silently fall through to Firestore dataURL save
      if (e.message !== 'STORAGE_TIMEOUT') {
        console.error('Storage error detail:', e);
      }
      // Fall through to Firestore-with-dataURL below
    }
  }

  // ── Firestore save with dataURL image (Storage not available or failed) ──
  if (fbReady && db) {
    try {
      showProgress('Saving to cloud…', 60);
      ...
      showProgress('Firestore error — saving locally…', 70);
    }
  }
...
function updateStats(games) {
  ...
  const sd = document.getElementById('statDevs');
  const sdl = document.getElementById('statDl');
  if (!sg) return;
  const devs = new Set(games.map(g => g.developer).filter(Boolean));
  const totalDl = games.reduce((a, g) => a + (g.downloads || 0), 0);
  // Animate numbers counting up
  function animateNum(el, target) {
    const start = parseInt(el.textContent) || 0;
    const diff = target - start;
    if (diff === 0) return;
    let step = 0; const steps = 24;
  ...
  animateNum(sg, games.length);
  animateNum(sd, devs.size);
  animateNum(sdl, totalDl);
}
...
async function renderCategoryRows() {
  ...
  cats.forEach(cat => {
    ...
    if (!inCat.length) {
      const empty = document.createElement('div');
      empty.style.cssText = 'display:flex;align-items:center;gap:10px;padding:18px 20px;color:rgba(255,255,255,.22);font-size:.74rem;font-family:Orbitron,sans-serif;letter-spacing:.08em;';
      empty.innerHTML = '<span style="font-size:1.2rem;opacity:.4">🎮</span> No games yet — be the first!';
  ...
  submitForm.addEventListener('submit', async ev => {
    ...
    try {
      ...
      if (ok) {
        ...
        // saveGame returned false — already showed error, just reset UI
        hideProgress();
      }
    } catch(err) {
      console.error('Submit error:', err);
      hideProgress();
      alert('Error: ' + err.message + '\n\nTry a smaller image (under 2MB) or a different format.');
    } finally {
      // ALWAYS re-enable button — no matter what happened
